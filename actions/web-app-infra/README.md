...
The **apply** step acquires the lock normally. If it encounters an existing lock it will fail and report the lock details (ID, operation, timestamp) — use those to determine whether the lock is stale before force-unlocking.
